...
def _experiment_commentary(
    ...
    """Build an HTML commentary block for this experiment.

    Emits a computed C-index delta line, then a placeholder for hand-authored
    analysis to be filled in after reviewing results.
    """
    ...
    if baseline_ci is not None:
        ...
        if delta > 0.005:
            ...
            verdict = f"underperforms the baseline by {abs(delta):.4f}"
        paragraphs.append(f"Mean C-index {ci_mean:.4f} — this {verdict} ({baseline_ci:.4f}).")

    paragraphs.append('<em class="muted">Analysis pending.</em>')
