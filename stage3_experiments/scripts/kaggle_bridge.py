...
def _make_fetch_data_cell(dataset_slug: str | None = None) -> str:
    ...
    return (
        ...
        "print(f'/kaggle/input/ tree:')\n"
        "for _p in sorted(_input_root.rglob('*')):\n"
        "    print(f'  {_p.relative_to(_input_root)}')\n"
        "\n"
        ...
        "    _src = None\n"
        "\n"
        "    # 1. Try the expected slug path (flat layout: /kaggle/input/<slug>/).\n"
        "    if _DATASET_SLUG and (_input_root / _DATASET_SLUG).exists():\n"
        "        _src = _input_root / _DATASET_SLUG\n"
        "        print(f'Using mounted dataset (flat): {_src}')\n"
        "\n"
        "    # 2. Recursive auto-discover: walk the full /kaggle/input/ tree.\n"
        "    #    Kaggle sometimes nests datasets under datasets/<owner>/<slug>/\n"
        "    #    or datasets/<slug>/ rather than at the top level.\n"
        "    if _src is None:\n"
        "        for _d in sorted(_input_root.rglob('*')):\n"
